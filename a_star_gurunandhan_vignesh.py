...
def pygame_visualization(visited_nodes, shortest_path, obstacle_points, hexagon_points, triangle_points):
    ...
    while condition:
        ...
        for node in visited_pts:
            if node in parent_child_info:
                child = parent_child_info[node]
                node = flip_points(node, 250)
                child = flip_points(child, 250)
                draw_arrow(window, "yellow", "white",
                      [node[0], node[1]], [child[0], child[1]], 1)
                pyg.display.flip()
                clock.tick(300)
        for points in shortest_path:
            pyg.draw.circle(window, (0, 255, 255), flip_points(points, 250), radius)
            pyg.display.flip()
            clock.tick(10)
        condition = False
    pyg.quit()
